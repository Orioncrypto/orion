...
# docker build --build-arg cores=4 -t orion/devbuilds:3.9.10 .
FROM ubuntu:trusty
...
# Copy source files
COPY . /opt/orion/BlockDX/

# Build source
RUN mkdir -p /opt/orion/BlockDX \
  && mkdir -p /opt/blockchain/config \
  && mkdir -p /opt/blockchain/data \
  && ln -s /opt/blockchain/config /root/.orion \
  && cd /opt/orion/BlockDX \
  ...
  && make -j$ecores \
  && make install \
  && rm -rf /opt/orion/ /tmp/berkeley/*

# Write default orion.conf (can be overridden on commandline)
RUN echo "datadir=/opt/blockchain/data    \n\
                                          \n\
dbcache=256                               \n\
maxmempool=512                            \n\
                                          \n\
port=6668    # testnet: 41474            \n\
rpcport=6667 # testnet: 41419            \n\
                                          \n\
listen=1                                  \n\
server=1                                  \n\
maxconnections=100                        \n\
...
rpcclienttimeout=15                       \n\
rpcuser=test                              \n\
rpcpassword=user" > /opt/blockchain/config/orion.conf

WORKDIR /opt/blockchain/
VOLUME ["/opt/blockchain/config", "/opt/blockchain/data"]

# Port, RPC, Test Port, Test RPC
EXPOSE 6668 6667 41474 41419
