...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i Orion-Core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp bitcoin-$HOST/src/qt/orion-qt $OUTDIR/bin/ || cp bitcoin-$HOST/src/qt/orion-qt.exe $OUTDIR/bin/ || echo "no orion wallet binaries!"
    cp bitcoin-$HOST/src/oriond $OUTDIR/bin/ || cp bitcoin-$HOST/src/oriond.exe $OUTDIR/bin/ || echo "no orion daemon binaries!"
    cp bitcoin-$HOST/src/orion-cli $OUTDIR/bin/ || cp bitcoin-$HOST/src/orion-cli.exe $OUTDIR/bin/ || echo "no orion-cli binaries!"
