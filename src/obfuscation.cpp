    ...
    if (fMasternode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Orion and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 ZRN per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeOrionAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
        nFeeRet, strFail, &coinControl, ONLY_NONDENOMINATED_NOT_MASTERNODE_REQUIRED_AMOUNT_IFMN);
    if (!success) {
        ...
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, strFail, coinControlNull, ONLY_NOT_MASTERNODE_REQUIRED_AMOUNT_IFMN);
        if (!success) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("orion-obfuscation");
