...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OrionMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OrionMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("OrionMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("orion-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running OrionMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
