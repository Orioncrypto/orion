...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 1062500 * COIN; 
    } else if (nHeight < 85000 && nHeight > 0) {
        nSubsidy = 40 * COIN;
    } else if (nHeight < 275000 && nHeight > 85000) {
        nSubsidy = 36 * COIN;
	} else if (nHeight < 450000 && nHeight > 275000) {
        nSubsidy = 28 * COIN;
        ...
        nSubsidy = 2 * COIN;
	} else if (nHeight < 3231568 && nHeight > 4000000) {
        nSubsidy = 1 * COIN;
	}
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60);// && chainActive.Height() > 500; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.nVersion >= 3 && pindex->nHeight >= 2000 &&
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    // TODO : ENABLE ZRN CACHE IN SPECIFIC CASES
