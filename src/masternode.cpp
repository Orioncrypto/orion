...
std::string CMasternode::GetConnectedWalletsStr() const
{
    ...
    std::string result = boost::algorithm::join(connectedWallets |
                                                boost::adaptors::transformed([](const CMasternodeXWallet & item) { return item.strWalletName;}),
                                                separator);
...
CMasternodeBroadcast::CMasternodeBroadcast(const CService & newAddr,
                                             const CTxIn & newVin,
                                             ...
                                             const std::vector<string> & exchangeWallets)
{
    ...
    for(const std::string & walletName : exchangeWallets)
        connectedWallets.push_back(CMasternodeXWallet(walletName));
}
...
bool CMasternodeBroadcast::Create(const string & strService,
                                   const string & strKeyMasternode,
                                   ...
                                   const bool fOffline)
{
    ...
    return Create(txin, CService(strService), keyCollateralAddressNew,
                  pubKeyCollateralAddressNew, keyMasternodeNew,
                  pubKeyMasternodeNew, exchangeWallets,
                  strErrorRet, mnbRet);
}
                                   ...
                                   CMasternodeBroadcast & mnbRet)
{
    ...
    mnbRet = CMasternodeBroadcast(service, txin, pubKeyCollateralAddressNew,
                                   pubKeyMasternodeNew, PROTOCOL_VERSION, exchangeWallets);
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 6668) return false;
    } else if (addr.GetPort() == 6668)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 ZRN tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 Orion tx -> 1 confirmation
...
CMasternodePing::CMasternodePing() : sigTime(0)
{
}
