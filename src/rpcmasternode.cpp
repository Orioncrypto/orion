...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <orionaddress> <amount>\n"
            "orionaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <orionaddress> <amount>\n"
            "orionaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in orion.conf\n"
        ...
        BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            if (mne.getAlias() == alias) {
                ...
                if (!snodeTx.IsNull() && mne.castOutputIndex(snodeTxIdx) && static_cast<int>(snodeTx.vout.size()) > snodeTxIdx) {
                    ...
                    if (snodeTxAmount != MASTERNODE_REQUIRED_AMOUNT*COIN) {
                        statusObj.push_back(Pair("result", "failed"));
                        statusObj.push_back(Pair("errorMessage", strprintf("Masternode input requires %d ZRN. Only %f ZRN was found",
                                                         MASTERNODE_REQUIRED_AMOUNT, (float)snodeTxAmount/(float)COIN)));
                        break;
                    }
                    ...
                    if (age < MASTERNODE_MIN_CONFIRMATIONS) {
                        statusObj.push_back(Pair("result", "failed"));
                        statusObj.push_back(Pair("errorMessage", strprintf("Masternode input requires %d confirmations. It only has %d",
                    ...
                    statusObj.push_back(Pair("result", "failed"));
                    statusObj.push_back(Pair("errorMessage", "Masternode input collateral is invalid."));
                }
    ...
    if (strCommand == "start-many" || strCommand == "start-all" || strCommand == "start-missing" || strCommand == "start-disabled") {
        ...
        BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            if (success) {
                ...
                if (snodeTxAmount != MASTERNODE_REQUIRED_AMOUNT * COIN) {
                    success = false;
                    errorMessage = strprintf("Masternode input requires %d ZRN. Only %f ZRN was found",
                                                       MASTERNODE_REQUIRED_AMOUNT, (float) snodeTxAmount / (float) COIN);
                }
                ...
                if (success) {
                    ...
                    if (age < MASTERNODE_MIN_CONFIRMATIONS) {
                        success = false;
                        errorMessage = strprintf("Masternode input requires %d confirmations. It only has %d",
                                                           MASTERNODE_MIN_CONFIRMATIONS, age);
                    }
                }
                // Register masternode with network
                if (success) {
                    activeMasternode.status = ACTIVE_MASTERNODE_INITIAL;
                    success = activeMasternode.Register(mne.getIp(), mne.getPrivKey(), mne.getTxHash(), mne.getOutputIndex(), errorMessage);
                }
            } else {
                errorMessage = "Masternode input collateral is invalid.";
            }
    ...
    if (strCommand == "status") {
        ...
        throw runtime_error("Masternode has not been verified by the network yet. This typically takes 5-10 minutes after start.\n");
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1)) {
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",         (string) Masternode Orion address\n"
