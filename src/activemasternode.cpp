...
//
// Bootup the Masternode, look for a 5000 Orion input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    // If state is set to true, masternode ping will be sent regardless of last occurrence.
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != 6668) {
                notCapableReason = strprintf("Invalid port: %u - only 6668 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if (service.GetPort() == 6668) {
            notCapableReason = strprintf("Invalid port: %u - 6668 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
    ...
    //send to all peers
    if (!SendMasternodePing(errorMessage, forceSendPing)) {
...
std::string CActiveMasternode::GetStatus()
{
    ...
    case ACTIVE_MASTERNODE_INITIAL:
        return "Masternode started, activation in progress...";
...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage, bool force)
{
    if (status != ACTIVE_MASTERNODE_STARTED) {
    ...
    if (pmn != NULL) {
        ...
        if (!force && pmn->IsPingedWithin(MASTERNODE_PING_SECONDS, mnp.sigTime)) {
        ...
        BOOST_FOREACH (CNode* pnode, vNodes)
            pnode->PushMessage("dseep", vin, vchMasternodeSignature, serviceNodeSignatureTime, false);
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != 6668) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only 6668 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == 6668) {
        errorMessage = strprintf("Invalid port %u for masternode %s - 6668 is only supported on mainnet.", service.GetPort(), strService);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
...
bool CActiveMasternode::Register(CTxIn vin, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, std::string& errorMessage)
{
    ...
    mnb = CMasternodeBroadcast(service, vin,
                                pubKeyCollateralAddress, pubKeyMasternode,
                                PROTOCOL_VERSION,
    ...
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
        pnode->PushMessage("dsee", vin, service, vchMasternodeSignature, serviceNodeSignatureTime, pubKeyCollateralAddress, pubKeyMasternode, -1, -1, serviceNodeSignatureTime, PROTOCOL_VERSION, donationAddress, donationPercantage);
...
bool CActiveMasternode::GetMasternodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if (possibleCoins.empty()) {
        LogPrintf("CActiveMasternode::GetMasternodeVin - No coins, could not locate valid vin\n");
        return false;
    }
...
bool CActiveMasternode::EnableHotColdMasternode(CTxIn& newVin, CService& newService)
{
    ...
    // update xbridge info for my masternode
    ...
    if (mn)
    {
        ...
        if (e.isEnabled())
        {
            ...
            for(const std::string & walletName : e.connectedWallets())
                mn->connectedWallets.push_back(CMasternodeXWallet(walletName));

            CMasternodeBroadcast mnb(*mn);
            uint256 hash = mnb.GetHash();
            if (mnodeman.mapSeenMasternodeBroadcast.count(hash))
            {
                mnodeman.mapSeenMasternodeBroadcast[hash].connectedWallets = mn->connectedWallets;
            }
        }
    }
