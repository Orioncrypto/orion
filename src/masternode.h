...
class CMasternodeXWallet
{
public:
    explicit CMasternodeXWallet() {}
    explicit CMasternodeXWallet(const std::string & walletName) : strWalletName(walletName) {}
...
//
// The Masternode Class. For managing the Obfuscation process. It contains the input of the 5000 ZRN, signature to prove
...
public:
    ...
    // xbridge wallets list, connected to masternode
    std::vector<CMasternodeXWallet> connectedWallets;
...
public:
    ...
    CMasternodeBroadcast(const CService & newAddr,
                          const CTxIn & newVin,
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        ...
        READWRITE(lastPing);
        READWRITE(nLastDsq);
        if (nType == SER_NETWORK && nVersion >= MASTERNODE_WITH_XBRIDGE_INFO_PROTO_VERSION)
        {
